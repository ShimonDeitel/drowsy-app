import XCTest
@testable import Drowsy

final class DrowsyTests: XCTestCase {
    var store: Store!
    ...
    @MainActor func testFreeLimitBlocksAdd() {
        for _ in 0..<Store.freeLimit {
            _ = store.add(SleepEntry(), isPro: false)
        }
        let blocked = store.add(SleepEntry(), isPro: false)
    ...
    @MainActor func testProBypassesLimit() {
        for _ in 0..<Store.freeLimit {
            _ = store.add(SleepEntry(), isPro: true)
        }
        let added = store.add(SleepEntry(), isPro: true)
        XCTAssertTrue(added)
    }
